...
export const SettingsView: React.FC = () => {
  ...
  const handleBugReport = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const device = formData.get('device')?.toString() || "Not mentioned";
...
Device: ${device}`;
    ...
    const url = `https://wa.me/918721923320?text=${encodeURIComponent(message)}`;
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white dark:bg-slate-800 rounded-[16px] p-5 border border-[#FFE4EA] dark:border-slate-700 shadow-sm relative overflow-hidden group">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                <h3 className="text-[16px] font-bold text-brand-gradient font-display mb-1">Support WorQ-Ai</h3>
                <p className="text-[13px] text-[#6B7280] dark:text-gray-400 leading-relaxed font-medium">
                  If WorQ-Ai helps you, consider supporting us with a small voluntary donation.
                </p>
              </div>
            </div>
            ...
            <a 
              href={`upi://pay?pa=nanu.adarsh@ptaxis&pn=Filemint&am=${selectedAmount}&tn=Filemint%20App%20Donation`}
              className="w-full flex items-center justify-center gap-2 py-3.5 mb-4 rounded-xl bg-[#111827] dark:bg-white text-white dark:text-[#111827] font-bold text-[14px] active:scale-95 transition-all shadow-md"
            >
              Donate ₹{selectedAmount} via UPI
            </a>
          ...
          {/* BLOG SECTION */}
          <div>
            <h3 className="text-[12px] font-bold text-[#6B7280] dark:text-gray-500 uppercase tracking-wider mb-2 px-1">Blog / Guides</h3>
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-[#E5E7EB] dark:border-slate-700 overflow-hidden flex flex-col">
              <button 
                onClick={() => {
                  window.history.pushState({}, '', '/blog/merge-pdf-online-free');
                  window.dispatchEvent(new Event('popstate'));
                }}
                className="w-full flex items-center justify-between px-4 py-4 hover:bg-brand-light/20 dark:hover:bg-slate-700/50 transition-colors group text-left border-b border-gray-100 dark:border-slate-700/50"
              >
                <div className="flex flex-col gap-1">
                  <span className="text-[15px] font-medium text-[#111827] dark:text-white leading-tight">Merge PDF Online Free (No Signup, Fast & Secure)</span>
                  <span className="text-[12px] text-[#6B7280] dark:text-gray-400">Combine PDF Files Easily in Seconds.</span>
                </div>
                <MdChevronRight className="text-[#E5E7EB] dark:text-slate-600 transition-transform group-hover:translate-x-1 shrink-0 ml-2" size={24} />
              </button>

              <button 
                onClick={() => {
                  window.history.pushState({}, '', '/blog/compress-pdf-without-losing-quality');
                  window.dispatchEvent(new Event('popstate'));
                }}
                className="w-full flex items-center justify-between px-4 py-4 hover:bg-brand-light/20 dark:hover:bg-slate-700/50 transition-colors group text-left border-b border-gray-100 dark:border-slate-700/50"
              >
                <div className="flex flex-col gap-1">
                  <span className="text-[15px] font-medium text-[#111827] dark:text-white leading-tight">Compress PDF Without Losing Quality</span>
                  <span className="text-[12px] text-[#6B7280] dark:text-gray-400">Reduce File Size Easily and Quickly.</span>
                </div>
                <MdChevronRight className="text-[#E5E7EB] dark:text-slate-600 transition-transform group-hover:translate-x-1 shrink-0 ml-2" size={24} />
              </button>
              ...
              >
                <div className="flex flex-col gap-1">
                  <span className="text-[15px] font-medium text-[#111827] dark:text-white leading-tight">PDF to Word Converter Free</span>
                  <span className="text-[12px] text-[#6B7280] dark:text-gray-400">Accurate & Fast way to convert your docs.</span>
                </div>
                <MdChevronRight className="text-[#E5E7EB] dark:text-slate-600 transition-transform group-hover:translate-x-1 shrink-0 ml-2" size={24} />
              </button>
              ...
              <button 
                onClick={() => {
                  window.history.pushState({}, '', '/blog/top-pdf-tools-2026');
                  window.dispatchEvent(new Event('popstate'));
                }}
                className="w-full flex items-center justify-between px-4 py-4 hover:bg-brand-light/20 dark:hover:bg-slate-700/50 transition-colors group text-left"
              >
                <div className="flex flex-col gap-1">
                  <span className="text-[15px] font-medium text-[#111827] dark:text-white leading-tight">Top 5 Free PDF Tools You Must Try in 2026</span>
                  <span className="text-[12px] text-[#6B7280] dark:text-gray-400">Discover the best tools for managing PDF files easily.</span>
                </div>
                <MdChevronRight className="text-[#E5E7EB] dark:text-slate-600 transition-transform group-hover:translate-x-1 shrink-0 ml-2" size={24} />
              </button>
            </div>
          ...
          <div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-[#E5E7EB] dark:border-slate-700 overflow-hidden divide-y divide-[#F8F9FC] dark:divide-slate-700/50">
              ...
              {[
                { id: 'about', label: 'About WorQ-Ai', icon: <MdInfo size={20} /> },
          ...
          >
            ...
            <header className="h-[64px] flex items-center px-4 border-b border-[#E5E7EB] dark:border-slate-800 bg-white dark:bg-slate-800">
              ...
              <h2 className="ml-2 text-[18px] font-bold text-[#111827] dark:text-white">
                {activeLegalPage === 'about' && 'About WorQ-Ai'}
              ...
              <div className="bg-white dark:bg-slate-800 rounded-[20px] p-6 border border-[#E5E7EB] dark:border-slate-700 shadow-sm prose prose-sm dark:prose-invert max-w-none">
                {activeLegalPage === 'about' && (
                  <div className="space-y-4">
                    <p className="text-[15px] leading-relaxed">WorQ-Ai is a fast and simple file utility app designed to help you manage PDFs, images, and signatures easily.</p>
                ...
                {activeLegalPage === 'privacy' && (
                  <div className="space-y-4">
                    <p className="text-[15px] leading-relaxed">WorQ-Ai does not upload or store your files on any server. All processing happens locally on your device.</p>
                ...
                {activeLegalPage === 'terms' && (
                  <div className="space-y-4 text-[15px] leading-relaxed">
                    <p>By using WorQ-Ai, you agree to use the app only for lawful purposes.</p>
                ...
                {activeLegalPage === 'disclaimer' && (
                  <div className="space-y-4 text-[15px] leading-relaxed">
                    <p>WorQ-Ai tools are provided for general purposes only.</p>
                ...
                {activeLegalPage === 'refund' && (
                  <div className="space-y-4 text-[15px] leading-relaxed">
                    <p>All donations made to WorQ-Ai are voluntary and non-refundable.</p>
