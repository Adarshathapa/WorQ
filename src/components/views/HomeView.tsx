  ...
  return (
    <div className="mb-6 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both">
      <div className="flex items-center justify-between mb-4 px-1">
        <h2 className="text-[clamp(16px,2vw,22px)] font-bold text-brand-gradient font-display tracking-tight">{title}</h2>
        {onSeeAll && (
...
export const HomeView: React.FC<{ onToolClick: (tool: ToolItem) => void }> = ({ onToolClick }) => {
  ...
  const quickToolMap = [
    ...
    { id: 'scan-to-pdf', label: 'Scan' },
    { id: 'remove-background', label: 'BG Remover' },
    { id: 'crop-rotate-image', label: 'Editor' },
  ...
  return (
    <div className="flex flex-col w-full max-w-[1400px] mx-auto p-4 md:p-8 gap-6 md:gap-10 bg-bg-base dark:bg-slate-950">
      ...
      {!isSearching && (
        <>
          {/* Quick Actions Grid */}
          <div className="flex flex-wrap justify-center gap-3 md:gap-6 lg:gap-10 px-1 w-full max-w-[1000px] mx-auto py-2">
            {quickTools.map((tool) => (
              <button 
                key={tool.id}
                onClick={() => onToolClick(tool as ToolItem)}
                className="flex flex-col items-center group active:scale-90 transition-transform w-[64px] md:w-[80px]"
              >
                <Icon name={tool.iconName} showContainer className="mb-2 group-hover:scale-105" />
                <span className="text-[11px] lg:text-[13px] font-bold text-[#6B7280] dark:text-gray-400 text-center leading-tight whitespace-nowrap overflow-hidden text-ellipsis w-full">
                  {tool.displayLabel}
                </span>
              </button>
            ))}
          ...
          {/* Ad Placeholder below quick actions */}
          <div className="w-full max-w-[1000px] mx-auto px-1">
             <AdPlaceholder id={1} />
          </div>
        </>
      )}
